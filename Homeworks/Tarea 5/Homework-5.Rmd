...
date: "24/06/2024"
course: "Fundamentos de programación y scriptyng"
---
...
- question_tags.csv: posee un identificador de etiqueta para cada pregunta y el ID dela pregunta.
...
Utilice `left_join()` en este ejercicio para asegurarse de mantener todas las preguntas, incluso aquellas sin un tag correspondiente.

## 1.1 Relacione questions y questions_tags usando las columnas id y question_id, respectivamente.
```{r}
questions %>% 
...
```
# 1.3 Utilice replace_na para cambiar los NA en la columna tag_name por "only-r".
```{r}
questions %>% 
  left_join(question_tags, by = c("id" = "question_id")) %>% 
  left_join(tags, by = c("tag_id" = "id")) %>% 
...
Realice un breve análisis, para ello utilice verbos de la familia dplyr como group_by, summarise, arrange y averigue el score promedio de las preguntas más frecuentes.
...
questions %>% 
  inner_join(answers, by = c("id" = "question_id"), suffix = c("_question", "_answer")) %>% 
  mutate(
    creation_date_question = ymd(creation_date_question),
    creation_date_answer = ymd(creation_date_answer)) %>%
  mutate(
    gap = creation_date_answer - creation_date_question)
```
# 4 Joining question and answer counts
Tambien podemos determinar cuántas preguntas realmente arrojan respuestas. Si contamos el número de respuestas para cada pregunta, podemos unir los conteos de respuestas con la tabla de preguntas.

## 4.1 Cuente y ordene la columna question_id en la tabla answers, luego almacene el resultado en la variable answer_counts..
```{r}
...
```
## 4.3 Reemplace los valores NA en la columna n con ceros 0.
```{r}
questions %>% 
  left_join(answer_counts, by = c("id" = "question_id")) %>% 
  mutate(n = replace_na(n, 0))
```
## 4.4 Por último almacene el resultado en la varible question_answer_counts.
```{r}
